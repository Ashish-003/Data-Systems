...
/**
 * @brief Function called top read a page from the buffer manager which is stored
 * during parthash join.
 *
 */
Page BufferManager::getPartition(string tableName, int key, int index, int partSize)
{
    string pageName = "../data/temp/" + tableName + "_part" + to_string(key) + "_page" + to_string(index);
    logger.log("BufferManager::getPartition");
    if (this->inPool(pageName))
        return this->getFromPool(pageName);
    else
        return this->insertPartitionIntoPool(tableName, key, index, partSize);
}
/**
...
Page BufferManager::insertPartitionIntoPool(string tableName, int key, int index, int partSize)
{
    logger.log("BufferManager::insertPartitionIntoPool");
    Page page(tableName, key, index, partSize);
    BLOCK_ACCESS_COUNT++;
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
