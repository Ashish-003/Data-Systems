...
TreePtr LeafNode::insert_key(const Key &key, const RecordPtr &record_ptr)
{
    ...
    if (this->overflows())
    {
        ...
        for (auto p : temp)
        {
            if (i <= ceil(FANOUT / 2))
            {
                // this->data_pointers[p.first] = p.second;
                this->data_pointers.insert(p);
                this->size++;
            }
            else
            {
                // new_leaf_node.data_pointers[p.first] = p.second;
                new_leaf_node->data_pointers.insert(p);
                new_leaf_node->size++;
            }
            i++;
        }
        TreePtr tempPtr = this->next_leaf_ptr;
        new_leaf = new_leaf_node->tree_ptr;
        this->next_leaf_ptr = new_leaf;
        new_leaf_node->next_leaf_ptr = tempPtr;
        new_leaf_node->dump();
    }
    this->dump();
    return new_leaf;
...
void LeafNode::delete_key(const Key &key)
{
    ...
    // cout << "leaf node delete is called" << endl;
