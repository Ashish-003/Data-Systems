...
class BufferManager{
    ...
    Page insertIntoPool(string tableName, int pageIndex, bool = false);

    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex, bool = false);
    void writePage(string pageName, vector<vector<int>> rows);
    void deleteFile(string tableName, int pageIndex);
    void deleteFile(string fileName);
    void clearPool();
    void writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount, bool = false);
};